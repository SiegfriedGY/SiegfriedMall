...
public class AttrServiceImpl extends ServiceImpl<AttrDao, AttrEntity> implements AttrService {
    ...
    public void saveAttr(AttrVo attr) {
        ...
        //2、保存关联关系
        // 销售属性不存在属性分组，所以只在基本属性（即规格参数）的情况下才保存分组
        ...
        List<AttrRespVo> respVos = records.stream().map((attrEntity) -> {
            ...
            if("base".equalsIgnoreCase(type)){
                AttrAttrgroupRelationEntity attrId = relationDao.selectOne(new QueryWrapper<AttrAttrgroupRelationEntity>()
                        .eq("attr_id", attrEntity.getAttrId()));    // 根据属性id到关联表里去查询属性分组id
    ...
    public PageUtils getNoRelationAttr(Map<String, Object> params, Long attrgroupId) {
        ...
        //2.1)、当前分类下的其他分组（其实应该是所有分组）
        List<AttrGroupEntity> group = attrGroupDao.selectList(new QueryWrapper<AttrGroupEntity>().eq("catelog_id", catelogId)
//                .ne("attr_group_id", attrgroupId) //这里有点绕，一开始觉得应该把自己这个分组排除掉，但当新建关联时，自己已经关联的属性，也不能显示在列表里，所以，这里不用ne，而是把自己分组也算进去，反而符合要求
        );
        ...
        QueryWrapper<AttrEntity> wrapper = new QueryWrapper<AttrEntity>().eq("catelog_id", catelogId)
                .eq("attr_type",ProductConstant.AttrEnum.ATTR_TYPE_BASE.getCode()); // 关联时，只能关联基本属性，即规格参数，而不能关联销售属性
        if(attrIds!=null && attrIds.size()>0){
